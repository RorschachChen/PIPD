...
def analyze_neuros(model, arch, num_classes, num_for_detect_biased, trainloader_no_shuffle,
    ...
    if arch == "nin":
        ...
        conv_list = ["Conv_0_", "Conv_2_", "Conv_5_", "Conv_7_", "Conv_10_", "Conv_12_", "Conv_14_", "Conv_17_",
                     "Conv_19_", "Conv_21_", "Conv_24_", "Conv_26_", "Conv_28_"]
        channel_num_list = [64, 64, 128, 128, 256, 256, 256, 512, 512, 512, 512, 512, 512]
    elif arch in ["resnet18", "resnet18_leakyrelu", "resnet18_elu", "resnet18_prelu", "resnet18_tanhshrink",
                  "resnet18_softplus"]:
        conv_list = [
            "Layer2_0_Conv1_",
    ...
    if arch in ["resnet18", "resnet18_leakyrelu", "resnet18_elu", "resnet18_prelu", "resnet18_tanhshrink",
                "resnet18_softplus", "efficientnet", "mobilenetv2", "densenet121"]:
        ...
        for data, target in loader:
            ...
            for i in range(len(conv_list)):
                conv_name = conv_list[i]
                if "FC" in conv_name:
                    conv_activation_all_list[i][counter:counter + batch_size] = inner_output_list[
                        i].cpu().detach().numpy()
                else:
                    conv_activation_all_list[i][counter:counter + batch_size] = inner_output_list[i].max(-1).values.max(
                        -1).values.cpu().detach().numpy()
            counter = counter + batch_size
    ...
    return poison_sample_index, poison_preset_indices, clean_preset_indices
